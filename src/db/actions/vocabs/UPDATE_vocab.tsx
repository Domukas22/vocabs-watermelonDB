...
export default async function UPDATE_vocab(incomingVocab: UpdateVocab_PROPS) {
  await db.write(async () => {
    ...
    await targetVocab.update((vocab: Vocab_MODEL) => {
    ...
    oldTRS?.forEach(async (oldTR: Translation_MODEL) => {
    ...
    newTRs?.forEach(async (newTR) => {
      ...
      if (
        !oldTRS?.some(
          (oldTR: Translation_MODEL) => oldTR.lang_id === newTR.lang_id
        )
      ) {
      ...
      if (
        ...
        if (targetTR) {
          await targetTR.update((tr: Translation_MODEL) => {
            tr.text = newTR.text;
            tr.highlights = newTR.highlights;
          });
        }
